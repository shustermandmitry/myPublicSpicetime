...
class JsonSchemaDefinition extends ForeginDefinition {
  ...
  constructor(jsonSchemas, schemaId, options) {
    ...
    this.jsonSchemaCollection =
      jsonSchemas instanceof Array
        ? jsonSchemas.reduce((schemas, curr) => {
            schemas[curr.id] = curr;
            return schemas;
          }, {})
        : undefined;
    ...
    this.jsonSchema =
      jsonSchemas instanceof Array ? this.jsonSchemaCollection[schemaId] : jsonSchemas;
    this.jsonSchemas = jsonSchemas instanceof Array ? jsonSchemas : [this.jsonSchema];
  ...
  compileProperty(propName) {
    ...
    const referencedSchema = this.jsonSchemaCollection
      ? this.jsonSchemaCollection[schemaProp.$ref]
      : undefined;
    ...
    if (referencedSchema || schemaProp.type === 'object' || schemaProp.type === 'array') {
      ...
      if (!isMixin) {
        // no need to care about sub-schema then, just use JSON field type
        // JSONB vs JSON: http://stackoverflow.com/a/33731703
        fieldDef.type = Sequelize.JSONB;
      } else {
        // Compile subschema into model defs
      ...
      switch (schemaProp.type) {
        case 'string':
          ...
          let type =
            schemaProp.maxLength && schemaProp.maxLength <= 255 ? Sequelize.STRING : Sequelize.TEXT;
          if (schemaProp.enum) {
        ...
        default:
          if (!customFieldDef) {
            ...
            throw new Error(
              `Unsupported JSON schema property type '${schemaProp.type}' for property '${propName}'`,
            );
          }
      }
    }
    ...
    if (typeof schemaProp.default !== 'undefined') {
      fieldDef.defaultValue = schemaProp.default;
    }
