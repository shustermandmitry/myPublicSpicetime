...
import { jsxToHtm, minify } from './jsx-parser';
  ...
  for (let i = 0; i < execCode.length; i++) {
    ...
    if (c === '<' && c2 !== '/') {
      ...
      if (!tags.length) {
        ...
        fixedCode +=
          'html`' +
          fixHtmlInnerCode(execCode.slice(start, end)).replace(/<([A-Z][\w\d_]*)/g, '<${$1}') +
          '`';
        prev = end;
        i = prev;
      }
...
export function loadScript(id: string, code: string, context: Context): Promise<LoadedScript> {
  ...
  (async function() {
  try {
    const { useCSS, require, html, add, jtd, Render, React, ...context } = __ls.context;
