...
const config = {
  ...
  port: {
    description: 'InnerPort for the runtime',
    default: 3569,
  },
  tlsKey: {
    cli: 'tls-key',
    description: 'Path to TLS key file',
    convert: fileName => fs.readFileSync(fileName),
  },
  tlsCert: {
    cli: 'tls-cert',
...
const commander = {
  version(ver) {
    return {
      option(cmd, desc, convert, def) {
        if (!def && typeof convert !== 'function') {
          def = convert;
          convert = undefined;
        }
        // XXX
...
function discoverIp(preferred) {
  ...
  if (typeof preferred === 'string' && ifaces[preferred]) {
    ...
    Object.keys(ifaces).find(iface => ifaces[iface].find(findInterface));
  }
...
const parseArguments = settings => {
  ...
  Object.keys(config).forEach(key => {
    ...
    const def = settings[key] ? undefined : conf.default;
    if (config[key].boolean) {
      options.option(`--${optionKey} [true]`, description, convertBoolean, def);
      return;
    }
    if (config[key].convert) {
      options.option(`--${optionKey} <${optionKey}>`, description, conf.convert, def);
      return;
    }
    options.option(`--${optionKey} <${optionKey}>`, description, def);
  });
...
const applyArguments = settings => {
  ...
  const options = parseArguments(settings);
  Object.keys(config).forEach(key => {
