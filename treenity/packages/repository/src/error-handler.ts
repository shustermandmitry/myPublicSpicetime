...
import { isWritableProperty } from './utils/get-property-descriptor';

const getDefaults = () => ({
  public: path.resolve(import.meta.dirname || '', '..', 'public'),
  logger: console,
});
const defaultHtmlError = path.resolve(getDefaults().public, 'default.html');
...
export function errorHandler(_options: ErrorHandlerOptions = {}): ErrorRequestHandler {
  const options = Object.assign({}, getDefaults(), _options);
  ...
  return function (error: any, req: Request, res: Response, next: NextFunction) {
    ...
    if (isWritableProperty(error, 'code')) {
    ...
    if (contentType.indexOf('json') !== -1 || accepts.indexOf('json') !== -1) {
      ...
      options.html &&
      (contentType.indexOf('html') !== -1 || accepts.indexOf('html') !== -1)
    ) {
