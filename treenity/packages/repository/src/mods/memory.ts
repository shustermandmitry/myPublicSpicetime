...
> extends AdapterBase<Result, Data, PatchData, ServiceParams, MemoryServiceOptions<Result>> {
  ...
  constructor(options: Partial<MemoryServiceOptions<Result>> = {}) {
    super({
      id: 'id',
      matcher: sift,
      sorter,
    ...
    this._uId = this.options.startId || 0;
  ...
  async _remove(
    ...
    if (id === null) {
      ...
      return Promise.all(
        entries.map((current: any) => this._remove(current[this.id] as Id, params)),
      );
    }
...
class MemoryServiceTreenity extends MemoryService {
  constructor(public params: any) {
    super(params.meta);
  }
}

feathersContext.add('sys.memory', MemoryServiceTreenity);
