...
export function getEntityDecorators(program: ts.Program, generator: JsonSchemaGenerator): DocEntry[] {
  ...
  function getSourceFileName(typeNode: ts.Identifier): string {
    ...
    if (ts.SymbolFlags.Alias & baseClassSymbol.flags) baseClassSymbol = checker.getAliasedSymbol(baseClassSymbol)!;
  ...
  function serializeClass(node: ts.ClassDeclaration) {
    ...
    const methodEntries = node.members
      ...
      .filter(method => {
        return ts.getDecorators(method)?.some(d => 
          d.expression.getText() === 'method' || d.expression.getText() === 'writeMethod'
        ) ?? false;
      })
      .map(method => {
        ...
        return [methodSymbol.getName(),
          {
            description: ts.displayPartsToString(methodSymbol.getDocumentationComment(checker)) || undefined,
            arguments: signature.parameters.map(param => ({
              name: param.getName(),
              description: ts.displayPartsToString(param.getDocumentationComment(checker)) || undefined,
  ...
  function getMetaType(node: ts.Node): { name?: string; type?: MetaType<any> } {
    ...
    if (
      ...
      if (!isExported) {
        console.warn(variableDeclaration.name.getText(), 'should be exported to generate schema in', variableDeclaration.getSourceFile().fileName);
      }
    ...
    if (ts.isCallExpression(node) && ts.isVariableDeclaration(node.parent)) {
      ...
      if (ts.isPropertyAccessExpression(expression) && expression.name.text === 'inContext') {
        ...
        if (ts.isIdentifier(object)) {
          ...
          const context = (node.arguments[0] as any).text;

          if (!context) {
            throw new Error(`wrong usage of 'inContext'`);
          }
          return { name: fileName + ':' + name, type: baseType.inContext(context) };
        }
      }
    }
      ...
      if (ts.isCallExpression(expr) && expr.expression.getText() === 'entity') {
        ...
        if (args.length >= 1) {
          ...
          if (ts.isStringLiteral(typeNode)) {
            ...
            const context = (args.length > 1 && ts.isStringLiteral(contextNode)) ? contextNode.text : '';
            ...
            if (!metaTypes[baseClass]) {
              throw new Error(`@entity decorator wrong usage, use: @entity(MyEntity|'my-entity', 'ctx') in ${fileName}`);
            }
