...
import { deepObserveWithUndoRedoPatches } from './observe-patches';
import type { Patch } from './undo-redo-patch';
...
class TestApp {
  constructor() {
    ...
    this._disposable = deepObserveWithUndoRedoPatches(this.document, (undo, redo) => {
  ...
  @observable accessor document: Doc = {
    name: 'steve',
    age: 93,
    address: {
...
describe('When replacing...', () => {
  ...
  it('produces the same result as fjp', async () => {
    ...
    app.patch(app.undos[0]);
    app.patch(app.redos[0]);
    expect(toJS(app.document)).toMatchObject(toJS(ctrl.document));
  });
});
